...
SCCf <- filter(SCC,str_detect(SCC.Level.One,'[Cc]omb'),str_detect(SCC.Level.Three,'[Cc]oal'),str_detect(SCC.Level.Four,'[Cc]oal')) 

# Opted not to filter SCC present through all the years in the assignment with SCCi
NEIf <- subset(NEI,SCC %in% SCCf$SCC)
...
ggplot(NEIf, aes(factor(year), Emissions), fill = type) + 
  ...
  labs(x = "Years",
       y = "Total Emissions",
       title = "Total Emissions in the US by Coal combustion-related sources")
dev.off()
