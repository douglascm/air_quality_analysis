...
SCCf <- filter(SCC,str_detect(SCC.Level.Two,'[Vv]ehic')) 

# Opted not to filter SCC present through all the years in the assignment with SCCi
NEIf <- subset(NEI,SCC %in% SCCf$SCC & fips %in% c("24510","06037"))
...
ggplot(NEIf, aes(factor(year), Emissions), fill = type) + 
  ...
  facet_wrap(. ~ fips, nrow = 1, ncol = 2,labeller = facet_labels) +
  labs(x = "Years",
       y = "Total Emissions",
       title = "Total Emissions between Los Angeles and Baltimore by Vehicle sources")
dev.off()
